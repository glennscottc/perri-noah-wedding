...
// ── CONTACT CARD ──────────────────────────────────────
function ContactCard({ vendorId, name, cat, contactName, phone, email, address, notes, status, isParents, onSave }) {
  const [editing, setEditing] = useState(false)
  const [f, setF] = useState({ contact_name: contactName||'', phone: phone||'', email: email||'', address: address||'', notes: notes||'' })
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)
  const catColors = {
    'Venue': ['#E6F1FB','#0C447C'], 'Florals': ['#EAF3DE','#27500A'],
    'Photography': ['#EEEDFE','#3C3489'], 'Music': ['#FAEEDA','#633806'],
    'Bands & Music': ['#FAEEDA','#633806'],
    'Catering': ['#FAECE7','#712B13'], 'Hair & Makeup': ['#FCEBEB','#791F1F'],
    'Transportation': ['#E1F5EE','#085041'],
  }
  const [cbg, ctc] = catColors[cat] || ['#f5f5f3','#6b6b68']
  const displayPhone = f.phone || phone
  const displayEmail = f.email || email
  const displayAddress = f.address || address
  const displayNotes = f.notes || notes
  const displayContact = f.contact_name || contactName

  async function save() {
    if (!onSave || !vendorId) return
    setSaving(true)
    await onSave(vendorId, f)
    setSaving(false); setSaved(true)
    setTimeout(() => { setSaved(false); setEditing(false) }, 1200)
  }

  return (
    <div style={{ background: 'white', borderRadius: 'var(--r-xl)', padding: '1rem 1.25rem', marginBottom: 12, border: '1px solid var(--border)', boxShadow: 'var(--shadow-xs)' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12, gap: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>{name}</div>
          {editing
            ? <input value={f.contact_name} onChange={e=>setF(p=>({...p,contact_name:e.target.value}))} placeholder="Contact name" style={{ fontSize:12, marginTop:4, width:'100%' }} />
            : displayContact && <div style={{ fontSize:12, color:'var(--text-secondary)', marginTop:2 }}>{displayContact}</div>
          }
        </div>
        <div style={{ display:'flex', gap:6, flexShrink:0, flexWrap:'wrap', justifyContent:'flex-end', alignItems:'center' }}>
          ...
          {onSave && <button onClick={()=>{setEditing(!editing);setF({contact_name:contactName||'',phone:phone||'',email:email||'',address:address||'',notes:notes||''})}} style={{fontSize:11,padding:'3px 10px',borderRadius:8,border:'1px solid var(--border-strong)',background:editing?'var(--rose-light)':'white',color:editing?'var(--rose-dark)':'var(--text-secondary)',cursor:'pointer',fontWeight:500}}>{editing?'Cancel':'✏️ Edit'}</button>}
        </div>
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {editing ? (
          <>
            {[['📞','Phone','phone','tel'],['✉️','Email','email','email'],['📍','Address','address','text'],['📌','Notes','notes','text']].map(([icon,label,key,type])=>(
              <div key={key} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:'#FEF9F5',borderRadius:10,border:'1px solid var(--rose-mid)'}}>
                <span style={{fontSize:18,flexShrink:0}}>{icon}</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:'var(--text-secondary)',marginBottom:4}}>{label}</div>
                  <input value={f[key]} onChange={e=>setF(p=>({...p,[key]:e.target.value}))} type={type} style={{fontSize:13,width:'100%'}} />
                </div>
              </div>
            ))}
            <button onClick={save} disabled={saving||saved} style={{width:'100%',padding:'11px',borderRadius:10,border:'none',background:saved?'#1D9E75':'var(--rose)',color:'white',fontSize:14,fontWeight:700,cursor:'pointer',marginTop:4}}>
              {saved?'✓ Saved!':saving?'Saving…':'Save changes'}
            </button>
          </>
        ) : (
          <>
            {displayPhone && <a href={`tel:${displayPhone.replace(/\D/g,'')}`} style={{display:'flex',alignItems:'center',gap:10,textDecoration:'none',padding:'10px 14px',background:'white',borderRadius:10,border:'0.5px solid var(--border)',transition:'background 0.15s'}} onMouseEnter={e=>e.currentTarget.style.background='#EEEDFE'} onMouseLeave={e=>e.currentTarget.style.background='white'}><span style={{fontSize:18,flexShrink:0}}>📞</span><div style={{flex:1}}><div style={{fontSize:11,color:'var(--text-secondary)',marginBottom:1}}>Phone</div><div style={{fontSize:14,fontWeight:500,color:'#0C447C'}}>{displayPhone}</div></div><span style={{fontSize:12,color:'var(--text-secondary)'}}>Tap to call</span></a>}
            {displayEmail && <a href={`mailto:${displayEmail}`} style={{display:'flex',alignItems:'center',gap:10,textDecoration:'none',padding:'10px 14px',background:'white',borderRadius:10,border:'0.5px solid var(--border)',transition:'background 0.15s'}} onMouseEnter={e=>e.currentTarget.style.background='#EEEDFE'} onMouseLeave={e=>e.currentTarget.style.background='white'}><span style={{fontSize:18,flexShrink:0}}>✉️</span><div style={{flex:1}}><div style={{fontSize:11,color:'var(--text-secondary)',marginBottom:1}}>Email</div><div style={{fontSize:14,fontWeight:500,color:'#0C447C'}}>{displayEmail}</div></div><span style={{fontSize:12,color:'var(--text-secondary)'}}>Tap to email</span></a>}
            {displayAddress && <a href={`https://maps.google.com/?q=${encodeURIComponent(displayAddress)}`} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:10,textDecoration:'none',padding:'10px 14px',background:'white',borderRadius:10,border:'0.5px solid var(--border)',transition:'background 0.15s'}} onMouseEnter={e=>e.currentTarget.style.background='#EEEDFE'} onMouseLeave={e=>e.currentTarget.style.background='white'}><span style={{fontSize:18,flexShrink:0}}>📍</span><div style={{flex:1}}><div style={{fontSize:11,color:'var(--text-secondary)',marginBottom:1}}>Address</div><div style={{fontSize:13,fontWeight:500,color:'var(--text-primary)'}}>{displayAddress}</div></div><span style={{fontSize:12,color:'var(--text-secondary)'}}>Maps ↗</span></a>}
            {displayNotes && <div style={{padding:'8px 14px',background:'white',borderRadius:10,border:'0.5px solid var(--border)',fontSize:12,color:'var(--text-secondary)',lineHeight:1.5}}>📌 {displayNotes}</div>}
            {!displayPhone && !displayEmail && !displayAddress && <div style={{fontSize:12,color:'var(--text-secondary)',fontStyle:'italic',padding:'4px 0'}}>No contact details yet — tap ✏️ Edit to add phone, email and address.</div>}
          </>
        )}
      </div>
    </div>
    ...
    <div style={S.section}>
      ...
      {viewMode === 'contacts' && (
        <div>
          ...
          {/* Old Oaks — always first, no edit needed */}
          <ContactCard
